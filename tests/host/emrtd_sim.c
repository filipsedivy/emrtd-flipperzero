...
static void
    ...
    if(sim->file_count >= SIM_FILE_MAX || len > SIM_FILE_SIZE) {
        return;
    }
...
static void sim_build_files(EmrtdSim* sim) {
    ...
    /*
     * EF.SOD. The security object is a CMS SignedData, and producing a signed
     * one would mean an ASN.1 writer and a key pair that this track has no use
     * for: nothing here verifies a signature. What the file has to be for the
     * reader's sake is a well formed outer template of a realistic size, so
     * that selecting it, sizing it from its header and reading it across
     * several frames are all exercised. That is what this is, and it is not a
     * security object.
     */
    uint8_t sod_body[128];
    memset(sod_body, 0x2A, sizeof(sod_body));
    sod_body[0] = 0x30;
    sod_body[1] = 0x81;
    sod_body[2] = (uint8_t)(sizeof(sod_body) - 3);
    uint8_t sod[160];
    const size_t sod_len = sim_write_do(sod, 0x77, sod_body, sizeof(sod_body));
    sim_add_file(sim, EMRTD_SIM_FID_SOD, 0x1D, sod, sod_len);
}
